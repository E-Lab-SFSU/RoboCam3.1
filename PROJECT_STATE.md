...
- **Well sub-labels**: right-clicking the grid opens a context menu to attach a free-text suffix ("treated", "ctrl", ...) to the clicked well, or to the whole current selection when the click lands inside it. Labelled cells render as two lines (well id over suffix) with a per-label accent colour derived from a stable hash of the label text, so a mis-assigned well in an otherwise uniform block is visible at a glance. The suffix is folded into the well label handed to `ExperimentRunner.run()` (`A1` → `A1-treated`), which puts it into every capture filename, the CSV, and the metadata JSON for free. Persisted in experiment presets and `session.json` as `{well_id: sub_label}`. Sanitisation (`robocam/naming.py`) folds anything non-alphanumeric to `-`, never `_` — see § 4. A **Clear Labels** button sits alongside Check All / Uncheck All / Invert: it's disabled when nothing is labelled, asks for confirmation, wipes every stored label (including any on wells outside the current plate size, which are retained but not drawn), and leaves the well *selection* untouched. The selected/labelled counter moved to its own row under the buttons, since four buttons plus the counter don't fit the column's 380px minimum width and a clipped `QLabel` doesn't elide.
